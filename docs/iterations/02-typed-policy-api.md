...
- Resources are policy-local names and cannot be referenced unless declared.
...
- `defineCachePolicy` is identity-style and supports inline objects without a class or DI.
- Runtime validation complements, but does not duplicate, compile-time guarantees.

## Functional scope

- Declare resources with `version`, `ttl`, and `key`.
...
4. Type exact-key derivation against the referenced resource's expected argument tuple.
...
6. Define `defineCachePolicy<T>()` without requiring explicit generic repetition beyond the
   provider contract.
7. Add runtime validation for empty names, unknown resources, invalid TTL/version, and
   contradictory rules.
...
- Compile-time negative fixtures for unknown methods/resources, synchronous configured
  methods, invalid argument tuples, read/mutation combinations, and unsupported refresh.
...
- Unsupported method shapes and resource references fail typechecking.
...
- Positive and negative type-test report
- Generated declaration review
- Unit test report for validation
- Minimal consumer example compiling without casts
